...
namespace coursework.src.Entities.Players
{
    public class Warrior : Player
    {
        ...
        private void SmashingBlade(List<Enemy> enemies)
        {
        ...
        {
            if(_level == _almightyPushLevel * 4)
            {
                Console.WriteLine("Almighty push level: +[1]");
                Console.WriteLine("Bloodlust level:     +[1]");
                _almightyPushLevel++;
                _bloodlustLevel++;
                Console.WriteLine($"Current almighty push level: [{_almightyPushLevel}]");
                Console.WriteLine($"Current bloodlust level:     [{_bloodlustLevel}]");
            }
        }
        protected override void CharacterBonuses()
        {
            double attackBonus = AttackLimit*0.05;
            Console.WriteLine($"Attack bonus: +[{attackBonus}]");
            Attack += attackBonus;
            Console.WriteLine($"Current attack:  [{Attack}]");
        }
